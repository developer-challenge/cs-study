...
메모리에 올라가는 시작 위치는 바뀌지만 안에 있는 코드상의 주소는 그대로 Logical address로 남아있음 → CPU는 Logical address를 바라봄
...
table이라는 자료구조 특성상 위에서부터 index를 통해 접근해야 하기 때문에 모든 entry가 만들어져야함
